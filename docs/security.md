...
A single-tenant Ubuntu/Nginx deployment is an explicit opt-in: set `JEVFLOW_PUBLIC_ORIGIN` to one canonical HTTPS origin (for example `https://flow.example.org`) and bind Node to **exactly** `127.0.0.1`. The server refuses malformed origins and any other bind host. In this mode it requires the HTTP `Host` to match that origin's host exactly, an exact HTTPS `Origin` for browser mutations, and a loopback TCP socket peer. A machine webhook may omit `Origin` only when its flow names a configured `JEVFLOW_HOOK_*` environment variable and the caller supplies its value in `x-jev-webhook-token`; the same token is required with a browser `Origin`. The reverse proxy still requires Basic Auth. It rejects duplicate Host/Origin headers and ignores `Forwarded` and `X-Forwarded-*` as authorization inputs. These checks constrain the request shape; **they do not authenticate a user**.
...
The separately hosted `jevflow.cloud` guide is deliberately public and static. Its build copies only allowlisted page source, six real screenshots, the WebM/VTT/transcript, generic `*.flow.json` examples, and `catalog-certification.json` into `site/dist`. The static guide cannot reach the loopback application or hold a provider key; do not add runtime traces, inputs, environment files, or model weights to its allowlist.
