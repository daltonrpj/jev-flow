...
Nginx must provide valid TLS and HTTP Basic Auth for the **entire** application, including `/`, `/jev`, `/api`, `/examples`, `/assets`, `/media`, docs, and the catalog certificate. There is no public static guide. Keep the Node port inaccessible from the network and keep the Basic password hash in `/etc/jev-flow/htpasswd`. Provider secrets belong in a root-owned `/etc/jev-flow/jev-flow.env`; application state belongs in `/var/lib/jev-flow`, not the checkout. Follow the [Hostinger VPS runbook](deploy-hostinger.md) and verify unauthenticated 401, authenticated Studio 200, and the loopback-only socket before announcing an instance.
