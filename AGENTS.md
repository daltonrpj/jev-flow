...
- Keep documentation English first. The Node application's `/` redirects to its Studio. The separate public `site/` guide is a static product guide served from the VPS; it must not expose runtime code, data, secrets, or imply that the public guide runs the application.
- Treat `site/index.html` as the canonical English source. `site:build` emits independent English, pt-BR, es, fr, and de pages from an explicit asset allowlist; translations must cover visible copy, alt text, accessibility labels, and metadata. Keep language links and assets relative. Do not translate user inputs, Flow JSON, provider results, or machine IDs.
- For every release or comparable update, append a dated entry to `docs/UPDATE-CONTEXT.md` so future work retains the product, release, verification, and deployment context. Update `CHANGELOG.md` for user-visible version releases. Keep `package.json` and `package-lock.json` versions aligned; use semantic versioning and tag the published release. Never record secrets or claim provider/deployment checks that were not actually performed.
